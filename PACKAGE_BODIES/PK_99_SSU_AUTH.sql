...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HCM"."PK_99_SSU_AUTH" AS 
    ...
    PROCEDURE PC_SSU_99_CHK (p_usr IN varchar2, p_cnt IN INT) AS 
		     ...
		     BEGIN 
			      ...
			      v_cnt := p_cnt; 
		           ...
		           IF v_cnt <> 1 THEN 
                     ...
                     UPDATE T_99_SSU_USR SET ius_yn = 0 WHERE UPPER(usr_id) = UPPER(p_usr) AND lgn_ctn > 3; 
     ...
     FUNCTION FN_99_SSU_AUTH (p_username IN varchar2, p_password IN varchar2) RETURN BOOLEAN 
         ...
         CURSOR c_comp_key IS SELECT comp_key FROM v_99_ssu_usr WHERE UPPER(usr_id) = UPPER(p_username); 
         ...
         BEGIN 
             ...
             v_valuesToHash  := apex_t_varchar2(v_comp_key,v_username, p_password); 
    ...
    -- Cloud user Authorization for pages 
        ...
        FUNCTION FN_99_SSU_PRIV RETURN BOOLEAN 
         ...
         CURSOR c_priv IS SELECT usr_rle FROM v_99_ssu_priv WHERE app_id = V('APP_ID') and pg_id = V('APP_PAGE_ID') AND acc_cd = V('APP_ACC_KEY'); 
         ...
         BEGIN 
             ...
             WHERE UPPER(usr_cd) = UPPER(V('APP_USER')) AND role_cd = v_usr_rle AND acc_cd = V('APP_ACC_KEY'); 
