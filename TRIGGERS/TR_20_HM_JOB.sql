--------------------------------------------------------
--  DDL for Trigger TR_20_HM_JOB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "HCM"."TR_20_HM_JOB" 
   BEFORE INSERT OR UPDATE ON t_20_hm_job 
   FOR EACH ROW 
 BEGIN 
   IF :new.r_k IS NULL THEN 
    -- SELECT to_number(sys_guid(),'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX') INTO :new.r_k FROM dual; 
    SELECT s_20_hm_job.NEXTVAL INTO :new.r_k FROM dual; 
   END IF; 
   IF inserting THEN 
...
/
ALTER TRIGGER "HCM"."TR_20_HM_JOB" ENABLE;
