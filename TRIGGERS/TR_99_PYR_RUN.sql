--------------------------------------------------------
--  DDL for Trigger TR_99_PYR_RUN
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "HCM"."TR_99_PYR_RUN" 
   BEFORE INSERT OR UPDATE ON t_99_pyr_run
   FOR EACH ROW 
 BEGIN 
   IF :new.r_k IS NULL THEN 
    -- SELECT to_number(sys_guid(),'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX') INTO :new.r_k FROM dual; 
    SELECT s_99_pyr_run.NEXTVAL INTO :new.r_k FROM dual; 
   END IF; 
   IF inserting THEN 
...
/
ALTER TRIGGER "HCM"."TR_99_PYR_RUN" ENABLE;
